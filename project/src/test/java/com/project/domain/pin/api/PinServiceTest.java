...
class PinServiceTest {
    ...
    private Point generatePoint(Double longitude, Double latitude) throws ParseException {
        return PointDTO.toPoint(new PointDTO(longitude, latitude));
    }
    ...
    public void get_pin_from_external_circle_not_allowed() {
        ...
        PinDTO.PinCreateRequest request1 = PinDTO.PinCreateRequest.builder().title("pin1").location(locationDTO).build();
        List<MultipartFile> pictures1 = List.of(generateFile("p1", "p1"),generateFile("p2", "p2"));
        PinDTO.PinDetailResponse createdPin = pinService.createPin(stranger, circle.getId(), request1, pictures1);
