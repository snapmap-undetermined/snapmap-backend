...
public class PinCommentController {
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "성공", content = @Content(schema = @Schema(implementation = PinCommentDTO.PinCommentDetailResponse.class)))})
    @Operation(summary = "핀 댓글 생성", description = "핀 댓글을 생성한다.")
    @SecurityRequirement(name = "Bearer Authentication")
    @PostMapping("/pin/{pinId}")
    @Permission
    public ResponseEntity<PinCommentDTO.PinCommentDetailResponse> createPinComment(@AuthUser Users user, @Parameter(description = "핀의 ID") @PathVariable Long pinId, @RequestBody PinCommentDTO.CreatePinCommentRequest request) {
        PinCommentDTO.PinCommentDetailResponse response = pinCommentService.createPinComment(user, pinId, request);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "성공", content = @Content(schema = @Schema(implementation = PinCommentDTO.PinCommentListResponse.class)))})
    @Operation(summary = "핀 댓글 리스트 조회", description = "핀에 속한 댓글 리스트를 조회한다.")
    @SecurityRequirement(name = "Bearer Authentication")
    @GetMapping("/pin/{pinId}/all")
    @Permission
    public ResponseEntity<PinCommentDTO.PinCommentListResponse> getPinCommentByPinId(@AuthUser Users user, @Parameter(description = "핀의 ID") @PathVariable Long pinId) {
    ...
    @Permission
    public ResponseEntity<PinCommentDTO.PinCommentDetailResponse> deletePinComment(@AuthUser Users user, @Parameter(description = "핀 댓글의 ID") @PathVariable Long pinCommentId) {
    ...
    @Permission
    public ResponseEntity<PinCommentDTO.PinCommentDetailResponse> deletePinCommentWithStatus(@AuthUser Users user, @Parameter(description = "핀 댓글의 ID") @PathVariable Long pinCommentId) {
    ...
    @Permission
    public ResponseEntity<PinCommentDTO.PinCommentDetailResponse> updatePinComment(@AuthUser Users user, @Parameter(description = "핀 댓글의 ID") @PathVariable Long pinCommentId, @RequestBody PinCommentDTO.UpdatePinCommentRequest request) {
